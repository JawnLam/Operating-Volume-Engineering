...
| **Convention 16 — Specialist Delegation** | `_meta/SPECIALISTS.md` (registry + detection triggers + sweep discipline); `TEMPLATE-specialist-requisition.md` (WHAT/WHY/constraints, out-of-scope declaration); mount contract in host manifests (`Sub_Artifact_Mounts`); `01-WHAT-IS-AN-OV.md § Composed OVs` | manual-only (Phase 3.13 walk) | F18 | The generalist holds only detection trigger + requisition interface; specialist internals never cross the boundary. First instance: ICME (status planned). Added v2.10.0. |
| **Specialist sweep execution** | Q7 sweep (`04-SCHEMA-DESIGN.md`); Step 4.7 (`BOOTSTRAP-NEW-OV.md`); Audit Mode §3b advisory (`03-DESIGN-PROTOCOL.md`); ship gate Phase 3.13 (HARD STOP — conditional) | manual-only (Phase 3.13 walk) | F18 | Every pipeline-shaped candidate classified `ov-native | requisition | hybrid-split`, classifications logged, negatives included. Added v2.10.0. |
