---
Item_Prototype: LFW_Character_Bible
Item_ID: "<character-slug>-bible"
Title: "<Character Name> — Bible"
lfw_Manuscript: ""
lfw_Item_Type: character-bible
lfw_Status: drafting            # drafting | established | revised | final
lfw_Character: ""               # wiki-link back to Character atom (e.g. [[Maya-Hollis]])
Date_Added:
Date_Modified:
Needs_Processing: false
---

# <Character Name> — Bible

> **The deep companion to the Character atom. Created on-demand for POV-bearing characters, antagonists, and primary supporting roles with substantial arcs. Most secondary characters never get one. Read in full only by CHARACTER-CONSISTENCY activity; otherwise read on-demand.**

## At-a-glance

*Single paragraph. The character in ~60 words. The summary loaded into context when other activities need a quick reference rather than the full bible.*

## Physical

- Appearance, posture, signature gestures
- Wardrobe / personal style
- The way they occupy a room
- Physical specifics that recur in the prose (smell, voice timbre, hands)

## Backstory (chronological)

*Birth-to-present, in dated bullets. The full timeline of a life. Granularity should match the manuscript's needs — every year for a novel that draws on the character's whole life; major moves only for a novel that lives in the present.*

- **Born:** *(year, place, into what family)*
- *(continue chronologically through to the manuscript's opening)*

## Family / lineage / ancestry

*The branches that matter. Family tree if non-trivial. Who's alive at manuscript opening; who's dead; who's estranged; who's distant.*

## Worldview

- **Core beliefs (load-bearing):**
- **Held-lightly beliefs:**
- **Politics** *(only if relevant)*:
- **Religious / spiritual orientation:**
- **What they think the world is fundamentally like:**

## Habits and routines

*Daily rhythms. What they do without thinking. The texture of their life. Where they wake; what they do first; what they eat; when they're most themselves.*

## Skills, knowledge, and competence

- **Expert at:**
- **Competent at:**
- **Publicly thought to be good at vs actually good at:** *(the gap matters — it's often subtext fuel)*
- **Skills the manuscript will draw on:**

## Wounds (deep)

*The injuries that shape them. The want/need/wound triad in the Character atom names ONE wound; the bible names all of them, including the ones that don't surface in this manuscript but inform the character's responses.*

- **Wound 1:**
- **Wound 2:**
- *(continue as needed)*

## Secrets

*What they're hiding, from whom, for how long, at what cost. Cross-reference with `_continuity.md` information-state ledger if the secrets are plot-relevant.*

- **Secret 1:** *(content, kept-from, since-when, current cost)*

## Contradictions

*The character's internal contradictions — what they value vs what they do; what they say vs what they want; what they've decided about themselves that the evidence doesn't support. The most important section for non-flat characters.*

## Arc across this manuscript

*Where they start, what changes, where they end. Beat-by-beat if useful. Cross-reference with the Character atom's "what's changing" section.*

## Relationships (per-relationship)

### With [[Other-Character-1]]

- **History:** *how they became who they are to each other*
- **Current state:** *what their relationship is at the manuscript's opening*
- **Evolution in this manuscript:**
- **Subtext:** *what's unspoken between them*

*(repeat per significant relationship)*

## Voice (extended — chapter 13 cross-reference)

*Building on the Character atom's voice-and-prose-register and dialogue-tells sections (chapter 13). Items here are ADDITIONAL to what's in the Character atom, not duplicative.*

- **Pet phrases not yet appearing in scenes** *(things the writer is reserving for the right moment)*
- **Verbal register at different ages** *(if the manuscript includes flashback or memory)*
- **How voice shifts under stress, intoxication, grief, joy:**

## Sensory signatures

*The character's smell, the sound of their walk, the specific way light hits them. Bookkeeping so the prose can draw on consistent sensory specifics across hundreds of pages.*

## Notes / not-yet-decided

*Open questions. Decision-deferred items. Things the writer wants to discover in drafting rather than predetermine here.*

---

## Audit notes

*CHARACTER-CONSISTENCY findings, updates after drift was surfaced, cross-references to Scene atoms where bible content is being honored or violated.*
